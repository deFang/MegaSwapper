...
describe('MegaSwapper', function () {
    ...
    before(async function() {
        ...
        impersonateAddress = "0xF2052f01d4E750542f54562D17d23d17380FBA29"
    ...
    it('swap', async function () {
        ...
        // await toke.connect(signer).approve(megaswapper.address, MAX)
        // console.log('before toke balance', (await toke.balanceOf(signer.address))/1e18)
        // txdata = await getTxdata(chainId, toke_address, eth_address, decimalStr("1000"), signer)
        ...
        // await megaswapper.connect(signer).swap(toke_address, eth_address, decimalStr("1000"), executor_address, executor_data)
        // console.log('after toke balance', (await toke.balanceOf(signer.address))/1e18)
        ...
        executor_data = txdata.data
        console.log("executor_address", executor_address, "executor_data", executor_data)
        await megaswapper.connect(signer).swap(usdc_address, wbtc_address, decimalStrUSDT("1000"), executor_address, executor_data)
    ...
    it ("swapETH", async function () {
        ...
        console.log('before btc balance', (await wbtc.balanceOf(signer.address))/1e8)
        txdata = await getTxdata(chainId, eth_address, wbtc_address, decimalStr("1"), signer)
        executor_address = txdata.to
        executor_data = txdata.data
        console.log("executor_address", executor_address, "executor_data", executor_data)
        await megaswapper.connect(signer).swapETH(wbtc_address, executor_address, executor_data, {value: decimalStr("1")})
    ...
    it("swapToETH", async function () {
        ...
        executor_data = txdata.data
        console.log("executor_address", executor_address, "executor_data", executor_data)
        await megaswapper.connect(signer).swap(usdc_address, eth_address, decimalStrUSDT("1000"), executor_address, executor_data)
        console.log('after eth balance', (await ethers.provider.getBalance(signer.address))/1e18)
    })
